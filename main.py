...
if uploaded_file:
    ...
    for idx, row in df.iterrows():
        ...
        table3 = [
            ...
            ["- 1번 항목(수레, 추의 질량)", row[25]],
            ["- 2번 항목(수레A+추0개, 수레B+추0개)", row[26]],
            ["- 3번 항목(수레A+추1개, 수레B+추0개)", row[27]],
            ["- 4번 항목(수레A+추2개, 수레B+추0개)", row[28]],
        ]
        pdf.add_table(table3, col_widths=col_widths_2col, merged_rows=[0])

        table4 = [
            ["(2-2) [4.결과 정리 및 해석] 관련 감점 사유", ""],
            ["- 5번 항목(정지상태의 운동량 합)", row[29]],
            ["- 6번 항목(운동상태의 운동량 합)", row[30]],
            ["- 7번 항목(수레 분열과정에서 변하는 물리량)", row[31]],
            ["- 8번 항목(수레 분열과정에서 변하지 않는 물리량)", row[32]],
            ["- 9번 항목(MaXa와 MbXb의 관계)", row[33]],
            ["- 10번 항목(실험결과와 운동량 보존이론 비교)", row[34]],
        ]
        pdf.add_table(table4, col_widths=col_widths_2col, merged_rows=[0])

        table5 = [
            ["(2-3) [5. 생각해보기] 관련 감점 사유", ""],
            ["- 11번 항목(실험-이론 불일치 이유1)", row[35]],
            ["- 12번 항목(실험-이론 불일치 이유2)", row[36]],
            ["- 13번 항목(오차 줄이는 방법1)", row[37]],
            ["- 14번 항목(오차 줄이는 방법2)", row[38]],
            ["- 15번 항목(일상에서 운동량 보존법칙이 적용된 예)", row[39]],
        ]
        ...
        table6 = [
            ...
            ["- 17번 항목(외력 없을 때 운동량 합 보존되는 이유)", row[41]],
        ]
